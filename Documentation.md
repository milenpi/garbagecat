...
Notes:
  ...
  1. A report called report.txt is created in the directory where the Garbage Cat tool is run.
...
Notes:
  ...
  1. Once prepocessing is done once, do not use the -p preprocessing option again, but instead specify the preprocessed file as input (e.g. java -jar /path/to/garbagecat-1.0.0.jar gc.log.pp).
...
```
  1. The bottom of the report shows Garbage Cat found 1000 (the maximum) unidentified lines:
```
...
========================================
